...
;; Version: 1.0.0
;; Package-Requires: ((emacs "28.1") (visual-fill-column "2.2") (async-http-queue "0.1.0"))
...
Uses `async-http-queue' for non-blocking, parallel downloads."
  (let* ((target-buffer (current-buffer))
    ...
    (dotimes (i nitem)
      (push (hackernews-modern--item-url (aref ids (+ offset i))) urls))
    (setq urls (nreverse urls))

    ;; Fetch items asynchronously using async-http-queue
    (async-http-queue
     urls
     :max-concurrent 10
