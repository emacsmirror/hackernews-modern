...
echo "=== HACKERNEWS-MODERN.EL QUALITY CHECKS ==="
echo ""
...
# Check all hackernews-modern*.el files
for file in hackernews-modern*.el; do
...
echo "  Linting hackernews-modern.el (main file)..."
emacs --batch \
      ...
      -f package-lint-batch-and-exit hackernews-modern.el 2>&1
...
if command -v docker >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then
    ...
    RECIPE='(hackernews-modern :fetcher github :repo "andros1983/hackernews-modern.el" :files ("*.el"))'
...
# Format all hackernews-modern*.el files
for file in hackernews-modern*.el; do
...
compile_file() {
    local file="$1"
    echo "  Compiling $file..."
    if emacs --batch --eval "(setq byte-compile-error-on-warn nil)" -f batch-byte-compile "$file" 2>&1 | grep -i "error\|warning" | grep -v "Loading"; then
